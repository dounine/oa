...
<script src="${base}/static/common/default/js/permission/list.js"></script>
<link href="${base}/static/common/default/css/permission/list.css" rel="stylesheet" type="text/css" />
</head>
<body style="padding: 10px;">
	<table class="table table-striped table-hover table-bordered" id="mainTable">
		...
		<thead>
			<tr>
				...
				<td class="span1" id="name">权限名称</td>
		...
		<tbody>
			<c:if test="${not empty resultSet}">
				<c:forEach var="permission" items="${resultSet.resultList}" varStatus="status">
					<tr id="row${status.index}" onselectstart="return false;" style="-moz-user-select: none">
						<td class="span1"><label class="checkbox"> <input type="checkbox" class="checkbox" />${status.count}</label> <input type="hidden" name="id" identity="true" value="${permission.id}" /></td>
						<td class="span1"><a href="javascript:doSaveOrUpdata('update', $('#row${status.index}'));">${fn:escapeXml(permission.name)}</a></td>
						<td class="span1">${fn:escapeXml(permission.code)}</td>
						<td class="span1">${fn:escapeXml(permission.descr)}</td>
						<c:choose>
							<c:when test="${fn:escapeXml(permission.disable) == 'Y' }">
								<td class="span1">是</td>
							</c:when>
							<c:otherwise>
								<td class="span1">否</td>
		...
		<tfoot>
			<!-- 分页 START -->
			<tr>
				<td colspan="5"><yahnc:pagelet url="${base}/permission/list.do?1=1" resultSet="${resultSet}" /></td>
			</tr>
			<!-- 分页 END -->
			<tr>
				<td colspan="5"><c:if test="${yahnfn:isRoles(sessionScope.login_user,fn:split(ADMIN_ROLE_CODE,','))}">
						<button class="btn btn-primary" data-toggle="modal" onclick="doSaveOrUpdata('save',this,0);">增加权限</button>
						<button class="btn btn-danger" data-toggle="modal" onclick="doDelete();">删除权限</button>
					</c:if>
	...
	<div class="modal hide fade" id="editDialog">
		<form action="save.do" method="post" id="saveForm" class="form-horizontal">
			...
			<div class="modal-header">
				...
				<h3 id="tableTitle">增加权限信息</h3>
			</div>
			<div class="modal-body">
				<table>
					<tbody>
						<tr>
							<td><label class="sr-only" for="name" style="width: 280px;">权限名称</label> <input type="text" class="form-control" name="name" placeholder="权限名称" /></td>
						...
						<tr>
							<td colspan="2"><label class="sr-only" for="descr">白名单地址</label> <input type="hidden" name="whiteUrls" id="whiteUrls" style="width: 500px;" clear="true" select2="true" /></td>
						</tr>
						<tr>
							<td colspan="2"><label class="sr-only" for="descr">黑名单地址</label> <input type="hidden" name="blackUrls" id="blackUrls" style="width: 500px;" clear="true" select2="true" /></td>
						</tr>
					</tbody>
				</table>
			</div>
	...
	<div class="modal hide fade" id="searchDialog">
		<form action="${base}/permission/list.do?1=1" method="post" id="searchForm" class="form-horizontal">
			<input type="hidden" id="url" value="${base}/permission/list.do?1=1" />
			<div class="modal-header">
				<a class="close" data-dismiss="modal">×</a>
		...
		<form action="${base}/permission/list.do?1=1" method="post" id="backupForm" style="display: none;">
