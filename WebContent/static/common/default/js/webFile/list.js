...
function doSaveOrUpdata(oper, caller, type) {
	if ("save" === oper) {
		...
		$("#editDialog #tableTitle").text("查看文件信息");
		$("#editDialog #url").val("/webFile/save.do");
		$('#editDialog').modal('show');
	} else {
		$("#editDialog #operType").val("update");
		$("#editDialog #tableTitle").text("查看文件信息");
		$("#editDialog #url").val("/webFile/update.do");
		var idObj = $(caller).find("input[type=hidden][name=id]");
		getData("/webFile/find.do", $("#editDialog"), idObj.val(), setAjaxForm);
		$('#editDialog').modal('show');
	}
}
...
function doDelete() {
	...
	if (0 < idObjs.length) {
		...
		$('#confirmDialog #confirmContext').html('是否确认删除这' + idObjs.length + '条记录<p style=\"color:red\">(注意:该操作将会物理删除改记录,并且物理删除指定文件!关联使用的文件也会被删除!)</p>');
		...
		$('#confirmDialog #confirmBtn').click(function() {
			...
			sendData(ids, deleteCallback, '/webFile/delete.do');
