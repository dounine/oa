...
public class TypeConverterDelegate {
	 ...
	 * @param requiredType
	 *            the type we must convert to (or {@code null} if not known, for example in case of a collection element)
	 * @param methodParam
	 *            the method parameter that is the target of the conversion (may be {@code null})
	...
	public <T> T convertIfNecessary(Object newValue, Class<T> requiredType, MethodParameter methodParam) throws IllegalArgumentException {
		...
		return convertIfNecessary(null, null, newValue, requiredType, (methodParam != null ? new TypeDescriptor(methodParam) : TypeDescriptor.valueOf(requiredType)));
	}
	 ...
	 *            the type we must convert to (or {@code null} if not known, for example in case of a collection element)
	 * @param field
	 *            the reflective field that is the target of the conversion (may be {@code null})
	 * @return the new value, possibly the result of type conversion
	 * @throws IllegalArgumentException
	 *             if type conversion failed
	 */
	public <T> T convertIfNecessary(Object newValue, Class<T> requiredType, Field field) throws IllegalArgumentException {
		...
		return convertIfNecessary(null, null, newValue, requiredType, (field != null ? new TypeDescriptor(field) : TypeDescriptor.valueOf(requiredType)));
	}
	...
	private Object doConvertValue(Object oldValue, Object newValue, Class<?> requiredType, PropertyEditor editor) {
		...
		if (requiredType != null && !requiredType.isArray() && convertedValue instanceof String[]) {
			...
			if (Number.class.isAssignableFrom(requiredType)) {
				String[] convertedValueArr = (String[]) convertedValue;
				String convertedNewValue = (convertedValueArr.length > 0) ? convertedValueArr[0] : "0";
				convertedValue = (org.apache.commons.lang3.StringUtils.isNotBlank(convertedNewValue) ? ((NumberUtils.isNumber(convertedNewValue) ? convertedNewValue : "0")) : null);
