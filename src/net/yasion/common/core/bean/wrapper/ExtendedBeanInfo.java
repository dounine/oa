 ...
 * </pre>
 * 
 * The standard JavaBeans {@code Introspector} will discover the {@code getFoo} read method, but will bypass the {@code #setFoo(Foo)} write method, because its non-void returning signature does not comply with the JavaBeans specification. {@code ExtendedBeanInfo}, on the other
 * hand, will recognize and include it. This is designed to allow APIs with "builder" or method-chaining style setter signatures to be used within Spring {@code <beans>} XML. {@link #getPropertyDescriptors()} returns all existing property descriptors from the wrapped
 * {@code BeanInfo} as well any added for non-void returning setters. Both standard ("non-indexed") and <a href="http://docs.oracle.com/javase/tutorial/javabeans/writing/properties.html"> indexed properties</a> are fully supported.
 *
 * @author Chris Beams
 * @since 3.1
...
public class ExtendedBeanInfo implements BeanInfo {
	...
	public static boolean isCandidateWriteMethod(Method method) {
		...
		return (methodName.length() > 3 && methodName.startsWith("set") && Modifier.isPublic(method.getModifiers()) && (!void.class.isAssignableFrom(method.getReturnType()) || Modifier.isStatic(method.getModifiers())) && (nParams == 1 || (nParams == 2 && parameterTypes[0]
				.equals(int.class))));
	}
	...
	/**
	 * Return the set of {@link PropertyDescriptor}s from the wrapped {@link BeanInfo} object as well as {@code PropertyDescriptor}s for each non-void returning setter method found during construction.
	 * 
	 * @see #ExtendedBeanInfo(BeanInfo)
	 */
	@Override
	...
	static class SimpleIndexedPropertyDescriptor extends IndexedPropertyDescriptor {
		...
		public boolean equals(Object other) {
			...
			return (ObjectUtils.nullSafeEquals(getIndexedReadMethod(), otherPd.getIndexedReadMethod()) && ObjectUtils.nullSafeEquals(getIndexedWriteMethod(), otherPd.getIndexedWriteMethod())
					&& ObjectUtils.nullSafeEquals(getIndexedPropertyType(), otherPd.getIndexedPropertyType()) && PropertyDescriptorUtils.equals(this, otherPd));
		}
		...
		@Override
		public String toString() {
			return String.format("%s[name=%s, propertyType=%s, indexedPropertyType=%s, " + "readMethod=%s, writeMethod=%s, indexedReadMethod=%s, indexedWriteMethod=%s]", getClass().getSimpleName(), getName(), getPropertyType(), getIndexedPropertyType(), this.readMethod,
					this.writeMethod, this.indexedReadMethod, this.indexedWriteMethod);
		}
	}

	/**
	 * Sorts PropertyDescriptor instances alpha-numerically to emulate the behavior of {@link java.beans.BeanInfo#getPropertyDescriptors()}.
	 * 
