...
import net.yasion.common.web.binder.SuperExtendedServletRequestDataBinder;
...
public class ExtendedModelAttributeMethodProcessor implements HandlerMethodArgumentResolver, HandlerMethodReturnValueHandler {
	 ...
	 */
	public ExtendedModelAttributeMethodProcessor(boolean annotationNotRequired) {
		this.annotationNotRequired = annotationNotRequired;
	}

	/**
	 * @return true if the parameter is annotated with {@link ModelAttribute} or in default resolution mode also if it is not a simple type.
	 */
	 ...
	 *             if data binding and validation result in an error and the next method parameter is not of type {@link Errors}.
	 * @throws Exception
	...
	public final Object resolveArgument(MethodParameter parameter, ModelAndViewContainer mavContainer, NativeWebRequest request, WebDataBinderFactory binderFactory) throws Exception {
		...
		WebDataBinder binderScr = binderFactory.createBinder(request, target, name);
		WebDataBinder binder = new SuperExtendedServletRequestDataBinder(target, name);
		AfxBeanUtils.copySamePropertyValue(binderScr, binder, null);
	...
	/**
	 * Whether to raise a {@link BindException} on bind or validation errors. The default implementation returns {@code true} if the next method argument is not of type {@link Errors}.
	 * 
	 * @param binder
